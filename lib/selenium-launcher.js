...
var override = process.env.SELENIUM_VERSION ? process.env.SELENIUM_VERSION.split(':') : []
  ...
  , expectedSha = override[1] || '9bc872d1f364a3104257b1f8e055a342228259c3'
...
function serverPort(cb) {
  if (process.env.SELENIUM_PORT) {
    return cb(undefined, process.env.SELENIUM_PORT);
  }
  freeport(cb);
}

function run(cb) {
  serverPort(function(er, port) {
    if (er) throw er;
...
module.exports = function(cb) {
  if (process.env.SELENIUM_LAUNCHER_PORT) {
    console.log('Faking Selenium Launch');
